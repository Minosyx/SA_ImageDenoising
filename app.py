        ...
        ui.panel_sidebar(
            ...
            ui.panel_conditional(
                ...
                ui.panel_conditional(
                    ...
                    ui.input_slider(
                        "wavelet_sigma",
                        "Divisor for the noise standard deviation",
                        min=1,
                        max=10,
                        step=0.1,
                        value=2,
                ...
                ui.input_slider(
                    "wavelet_levels", "Decomposition level", min=1, max=10, value=5
                ),
            ),
            ui.div(
...
def server(input: Inputs, output: Outputs, session: Session):
    ...
    def file():
        if input.file() is not None:
            ...
            img_size = ip.Utils.get_image_size(path)
            lesser = min(img_size)
    ...
    def get_wavelets():
        if input.denoise_method() == "wavelet":
            wavelets = ip.Utils.get_wavelets()
