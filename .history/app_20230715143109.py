    ...
    ui.layout_sidebar(
        ui.panel_sidebar(
            ...
            ui.panel_conditional(
                ...
                ui.input_numeric(
                    "fft_cutoff", "Cutoff", min=0, max=1, step=0.1, value=0.1
                ),
                ui.input_numeric(
                    "fft_threshold", "Threshold", min=0, max=1, step=0.1, value=0.5
                ),
            ),
            ui.div(
    ...
    def denoise():
        if input.denoise_method() is not None and img_original_path.get() is not None:
            ...
            args = {
                "svd": {"k": input.svd_k()},
                "fft": {
                    "cutoff": input.fft_cutoff(),
                    "threshold": input.fft_threshold(),
                },
            }
