...
class ImageDenoising(object):
    ...
    def svd(self, k: int):
        ...
        if len(image_shape) == 3:  # RGB image
            ...
            self.image = out_denoised.round().clip(0, 255).astype(np.uint8)  # Clip values to valid range
            return fft_dict
        else:  # Grayscale image
            img = self.image.astype(np.float32)
            F = fftn(img)
            ...
            self.image = denoised_img.round().clip(0, 255).astype(np.uint8)  # Clip values to valid range
