...
def server(input: Inputs, input: Outputs, session: Session):
    img_path = reactive.Value(None)
    input.denosing_ended = reactive.Value(False)
    input.denoised_img = reactive.Value(None)
    input.busy = reactive.Value(False)

    @input
    @render.image
    def out_img():
        if input.file() is not None:
            input.denosing_ended.set(False)
    ...
    def denoise():
        if input.denoise_method() is not None and img_path.get() is not None:
            ...
            input.denoised_img.set(img.denoise())
            input.denosing_ended.set(True)
