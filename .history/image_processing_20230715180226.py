...
class ImageDenoising(object):
    ...
    def wavelet(self, method: str, mode: str, wavelet_levels: int, wavelet: str, rescale_sigma: bool, sigma_reduction: float):
        image_shape = self.image.shape
        if len(image_shape) == 3:  # RGB image
            
        else:  # Grayscale image
            if method == 'BayesShrink':
                denoised_img = denoise_wavelet(self.image, method=method, mode=mode, wavelet_levels=wavelet_levels, wavelet=wavelet, rescale_sigma=rescale_sigma)
            elif method == 'VisuShrink':
                sigma_est = estimate_sigma(self.image, average_sigmas=True)
                denoised_img = denoise_wavelet(self.image, method=method, mode=mode, wavelet_levels=wavelet_levels, wavelet=wavelet, rescale_sigma=rescale_sigma, sigma=sigma_est / sigma_reduction)
