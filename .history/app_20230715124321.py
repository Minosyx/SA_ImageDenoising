...
def server(input: Inputs, output: Outputs, session: Session):
    ...
    def file():
        if input.file() is not None:
            ...
            img_original_path.set(input.file()[0]["datapath"])
    ...
    def denoise():
        if input.denoise_method() is not None and (
            img_path.get() is not None or img_old_path.get() is not None
        ):
            path = (
                img_old_path.get() if img_old_path.get() is not None else img_path.get()
            )
            img = ip.ImageDenoising(path, input.denoise_method())
            dimg = img.denoise()
