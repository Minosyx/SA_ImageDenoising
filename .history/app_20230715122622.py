...
def server(input: Inputs, output: Outputs, session: Session):
    ...
    @reactive.Effect
    @reactive.event(input.file, denosing_ended)
    @output
    @render.image
    def out_img():
        if input.file() is not None:
            if denoising_ended.get():
                ext = img_name.get().split(".")[-1]
                iio.imwrite(
                    "denoised_image.{ext}", denoised_img.get(), extension=f".{ext}"
                )
                file = {"src": "denoised_image.{ext}", "alt": "Denoised image"}
            else
